...
if EVAL_PROFILE_NAME == "smoke_3track":
    ...
    DEFAULT_RUN_LOG_NAME = "tuning_run_gp_bo_tv_smoke_1track.jsonl"
    DEFAULT_OUTPUT_CSV_NAME = "gpbo_tv_test_eval_smoke_3track.csv"
    DEFAULT_OUTPUT_CANDIDATES_NAME = "gpbo_tv_test_candidates_smoke_3track.json"
    DEFAULT_PLOTS_DIR_NAME = "gpbo_tv_eval_plots"

    # domyślne unstaged jak w poprzednim unstaged evaluatorze
    UNSTAGED_N_TRIALS = int(os.environ.get("EVAL_UNSTAGED_N_TRIALS", "60"))
    UNSTAGED_STARTUP = int(os.environ.get("EVAL_UNSTAGED_STARTUP", "25"))

elif EVAL_PROFILE_NAME == "night_7track":
    TRAIN_PROFILE_NAME = "night_4track"
    DEFAULT_EVAL_TRACK_SET = "8,9,10,11,12,13,14"

    DEFAULT_RUN_LOG_NAME = "tuning_run_gp_bo_tv_night_4track.jsonl"
    DEFAULT_OUTPUT_CSV_NAME = "gpbo_tv_test_eval_night_7track.csv"
    DEFAULT_OUTPUT_CANDIDATES_NAME = "gpbo_tv_test_candidates_night_7track.json"
    DEFAULT_PLOTS_DIR_NAME = "gpbo_tv_eval_plots"

    # domyślne unstaged jak w poprzednim unstaged evaluatorze
    UNSTAGED_N_TRIALS = int(os.environ.get("EVAL_UNSTAGED_N_TRIALS", "224"))
    UNSTAGED_STARTUP = int(os.environ.get("EVAL_UNSTAGED_STARTUP", "64"))
...
UNSTAGED_PHASE_NAME = os.environ.get("EVAL_UNSTAGED_PHASE_NAME", "unstaged_joint_gpbo")
...
# ============================================================
# Names retained from GPBO TV tuner
# ============================================================
...
# ============================================================
# Bounds / config retained for consistency with tuner TV
# ============================================================
SAFE_PLANNER = {
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11615"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_eval_gpbo_tv_{EVAL_PROFILE_NAME}_{GLOBAL_SEED}")
...
def _trial_order(rec: Dict[str, Any]) -> int:
    if "candidate_id" in rec:
        return int(rec["candidate_id"])
...
def load_jsonl(path: str) -> List[Dict[str, Any]]:
    ...
    with open(path, "r") as f:
        for idx, line in enumerate(f, start=1):
            ...
            out.append(rec)
    out.sort(key=lambda r: _trial_order(r))
    return out


def _is_startup_record(rec: Dict[str, Any]) -> bool:
    return _trial_order(rec) <= UNSTAGED_STARTUP
...
def _select_best_so_far_schedule(records: List[Dict[str, Any]]) -> List[EvalTask]:
    ...
    non_startup.sort(key=lambda r: _trial_order(r))
    ...
    for checkpoint_filtered in checkpoints_filtered:
        ...
        prefix_all = [r for r in records if _trial_order(r) <= checkpoint_global]
        prefix = [r for r in non_startup if _trial_order(r) <= checkpoint_global]
        if not prefix:
            continue
        ...
        max_seen_global = max(_trial_order(r) for r in prefix_all) if prefix_all else 0
        tail_padded = bool(max_seen_global < checkpoint_global)
        ...
        selected.append(
            EvalTask(
                ...
                source_trial_global=int(_trial_order(best_rec)),
...
def _select_algorithm_thinks_schedule(records: List[Dict[str, Any]]) -> List[EvalTask]:
    ...
    for checkpoint_filtered in checkpoints_filtered:
        ...
        available = [
            r for r in prefix
            if bool(r.get("surrogate_snapshot_available", False))
            and isinstance(r.get(field, None), dict)
            and isinstance(r.get(field, {}).get("theta", None), dict)
        ]
        if not available:
            continue

        source_rec = available[-1]
        snap = source_rec[field]
        ...
        selected.append(
            EvalTask(
                ...
                checkpoint_trial_filtered=int(checkpoint_filtered),
                checkpoint_trial_global=int(checkpoint_global),
                source_candidate_id=int(source_rec.get("candidate_id", source_rec["_global_trial"])),
                source_phase=str(source_rec.get("phase", "")),
                source_phase_trial=int(source_rec.get("phase_trial", -1)),
                source_trial_global=int(_trial_order(source_rec)),
                ...
                source_robust_cost_train=float(source_rec.get("robust_cost", _train_objective(source_rec))),
                source_best_objective_so_far_train=float(best_obj_so_far),
...
def _save_plot(rows: List[Dict[str, Any]], selection_family: str, y_key: str, y_label: str) -> None:
    ...
    plt.xlabel("Numer triala po odfiltrowaniu startupu")
    plt.ylabel(y_label)
...
def main() -> None:
    ...
    print("[INFO] algorithm_thinks uses the latest available surrogate snapshot up to each unstaged checkpoint.")
    print("[INFO] startup filtering is single-block, i.e. unstaged.")
    print("")
