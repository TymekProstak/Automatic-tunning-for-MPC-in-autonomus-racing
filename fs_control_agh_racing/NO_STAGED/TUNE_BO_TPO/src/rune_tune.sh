...
WS="${TUNE_WS:-$HOME/Desktop/fs_control_agh_racing/NO_STAGED/TUNE_BO_TPO}"

# Domyślny skrypt pod nowe custom-TPE 3-stage
PY_SCRIPT_REL="${TUNE_PY_SCRIPT_REL:-src/BO_TPO/tpo_fixed.py}"
...
# Dodatkowe pip pakiety opcjonalnie, np.
# export TUNE_EXTRA_PIP="numpy matplotlib"
...
RUN_LOG="$RUN_LOG_DIR/tuning_tpe_${STAMP}.log"
...
# ============================================================
# 5) Optional Python deps
# ============================================================
# Custom-TPE zwykle jedzie na stdlib, ale zostawiam opcję na przyszłość.
python3 - <<'PY'
import sys
try:
    import numpy  # noqa: F401
    print(f"[OK] numpy available: {numpy.__version__}")
except Exception:
    print("[INFO] numpy not required by runner; skipping install.")
PY
