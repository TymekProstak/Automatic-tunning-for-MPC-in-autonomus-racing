...
# Config
# ============================================================
WS="${EVAL_WS:-$HOME/Desktop/fs_control_agh_racing/NO_STAGED/TUNE_RANDOM_SEARCH}"

PY_SCRIPT_REL="${EVAL_PY_SCRIPT_REL:-src/random_search/eval.py}"
RUN_LOG_JSONL="${EVAL_RUN_LOG_PATH:-$WS/src/random_search/tuning_run_random_search_tv_unstaged_night_4track.jsonl}"
...
RUNNER_LOG="$RUN_LOG_DIR/eval_random_test_tracks_tv_${STAMP}.log"
...
export EVAL_ROS_MASTER_PORT="${EVAL_ROS_MASTER_PORT:-11337}"

# opcjonalne override'y:
# export EVAL_OUTPUT_CSV="$WS/src/random_search/random_search_test_eval_tv.csv"
...
# ============================================================
set +e
python3 "$PY_SCRIPT" 2>&1 | tee "$RUNNER_LOG"
PY_EXIT=${PIPESTATUS[0]}
set -e

echo "[RUN] Python exit code: $PY_EXIT"
echo "[RUN] Runner log saved to: $RUNNER_LOG"
