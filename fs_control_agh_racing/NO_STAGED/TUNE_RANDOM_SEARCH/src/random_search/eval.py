...
DEFAULT_CATKIN_WS = os.path.expanduser(
    "~/Desktop/fs_control_agh_racing/TUNE_RANDOM_SEARCH"
)
...
RUN_LOG_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_RUN_LOG_PATH",
        os.path.join(SCRIPT_DIR, "tuning_run_random_search_tv_unstaged.jsonl"),
    )
)
OUTPUT_CSV = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CSV",
        os.path.join(SCRIPT_DIR, "random_search_test_eval_tv_unstaged.csv"),
    )
)
...
# ============================================================
# Tuner config copied for consistency (UNSTAGED)
# ============================================================
PLANNER_BOUNDS = {
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11618"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_eval_random_unstaged_{GLOBAL_SEED}")
...
# ============================================================
# History / selection logic (UNSTAGED)
# ============================================================
...
def _best_so_far(history: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    if len(history) == 0:
        return None
    return min(history, key=lambda r: (float(r["mean_cost"]), int(r["_global_trial"])))
...
def _algorithm_thinks_theta(
    ...
    # W wersji unstaged bierzemy cały zbiór, znajdujemy G najlepszych i wyciągamy środek 
    # dla pełnej przestrzeni parametrów (ALL_BOUNDS).
...
def main() -> None:
    ...
    try:
        # ----------------------------------------------------
        # co 10 triali: single best-so-far
        # ----------------------------------------------------
        for checkpoint in range(BEST_SO_FAR_EVERY, len(records) + 1, BEST_SO_FAR_EVERY):
            history = records[:checkpoint]
            checkpoint_phase = "unstaged"

            best_rec = _best_so_far(history)
            if best_rec is None:
                continue

            print(
                f"\n[CHECKPOINT {checkpoint:03d}] best-so-far "
            ...
            row = _make_csv_row(
                ...
                selection_label="best_so_far",
                theta=theta,
                eval_out=eval_out,
                source_candidate_id=int(best_rec.get("candidate_id", -1)),
                source_phase=str(best_rec.get("phase", "unstaged")),
                source_mean_cost_train=float(best_rec.get("mean_cost", float("nan"))),
            )
            _append_csv_row(OUTPUT_CSV, CSV_FIELDS, row)

            print(
                f"  [best_so_far] cand_id={best_rec.get('candidate_id')} "
                f"train_cost={float(best_rec.get('mean_cost')):.6g} "
