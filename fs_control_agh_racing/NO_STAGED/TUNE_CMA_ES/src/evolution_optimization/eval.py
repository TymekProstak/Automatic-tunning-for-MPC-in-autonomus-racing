...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11617"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_test_eval_cma_tv_{PROFILE_NAME}_unstaged")
...
TEST_CANDIDATES_JSON = os.path.abspath(
    os.environ.get(
        ...
        os.path.join(SCRIPT_DIR, f"cma_tv_test_candidates_{PROFILE_NAME}_unstaged.json"),
    )
)
TEST_RESULTS_CSV = os.path.abspath(
    os.environ.get(
        "EVAL_RESULTS_CSV",
        os.path.join(SCRIPT_DIR, f"cma_tv_test_results_{PROFILE_NAME}_unstaged.csv"),
    )
)
PLOTS_DIR = os.path.abspath(
    os.environ.get(
        "EVAL_PLOTS_DIR",
        os.path.join(SCRIPT_DIR, f"cma_tv_eval_plots_{PROFILE_NAME}_unstaged"),
    )
)
...
def _distribution_full_theta(rec: Dict[str, Any]) -> Dict[str, float]:
    ...
    # W wersji unstaged dystrybucja to pełna przestrzeń, nie ma łączenia parametrów
    return {k: float(v) for k, v in rec.get("distribution_center_theta", {}).items()}
...
def _build_dist_records_with_global_trial_end(dist_log: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for rec in dist_log:
        ...
        # generation 0 / lambda 0 = init distribution snapshot, pomijamy
        if generation <= 0 or lambda_ <= 0:
            continue

        # W unstaged globalny trial to po prostu generation * lambda
    ...
    for checkpoint in range(CHECKPOINT_STEP, max_trial + 1, CHECKPOINT_STEP):
        ...
        selected.append(
            EvalTask(
                ...
                checkpoint_trial=int(checkpoint),
                source_phase=str(rec.get("phase", "unstaged")),
...
def _save_plot(rows: List[Dict[str, Any]], selection_family: str, y_key: str, y_label: str) -> None:
    ...
    # Usunięto plt.axvline (koniec stage) z wersji unstaged
