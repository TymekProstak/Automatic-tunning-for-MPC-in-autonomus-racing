...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11317"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_test_eval_cma_tv_{PROFILE_NAME}")
...
TEST_CANDIDATES_JSON = os.path.abspath(
    os.environ.get(
        ...
        os.path.join(SCRIPT_DIR, f"cma_tv_test_candidates_{PROFILE_NAME}.json"),
    )
)
TEST_RESULTS_CSV = os.path.abspath(
    os.environ.get(
        "EVAL_RESULTS_CSV",
        os.path.join(SCRIPT_DIR, f"cma_tv_test_results_{PROFILE_NAME}.csv"),
    )
)
PLOTS_DIR = os.path.abspath(
    os.environ.get(
        "EVAL_PLOTS_DIR",
        os.path.join(SCRIPT_DIR, f"cma_tv_eval_plots_{PROFILE_NAME}"),
    )
)
...
def _build_dist_records_with_global_trial_end(dist_log: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for rec in dist_log:
        ...
        # generation 0 / lambda 0 = init distribution snapshot, nie traktujemy jako checkpoint evaluate
