...
# Eval profiles
# ============================================================
EVAL_PROFILE_NAME = os.environ.get("EVAL_PROFILE_NAME", "night_7track")
...
RUN_LOG_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_RUN_LOG_PATH",
        os.path.join(SCRIPT_DIR, f"tuning_run_random_search_tv_{TRAIN_PROFILE_NAME}.jsonl"),
    )
)
OUTPUT_CSV_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CSV",
        os.path.join(SCRIPT_DIR, f"random_search_tv_test_eval_{EVAL_PROFILE_NAME}.csv"),
    )
)
OUTPUT_CANDIDATES_JSON = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CANDIDATES_JSON",
        os.path.join(SCRIPT_DIR, f"random_search_tv_test_candidates_{EVAL_PROFILE_NAME}.json"),
    )
)
PLOTS_DIR = os.path.abspath(
    os.environ.get(
        "EVAL_PLOTS_DIR",
        os.path.join(SCRIPT_DIR, f"random_search_tv_eval_plots_{EVAL_PROFILE_NAME}"),
    )
)
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11318"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_eval_random_search_tv_{EVAL_PROFILE_NAME}")
...
def _best_record(records: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    return min(
        records,
        key=lambda r: (_train_objective(r), int(r.get("_global_trial", 10**18))),
    )
...
# ============================================================
# Random-search "algorithm thinks" helpers
# ============================================================
...
def _weighted_center_from_records(
    ...
    ranked = sorted(recs, key=lambda r: _train_objective(r))
...
def _top_records(records: List[Dict[str, Any]], k: int) -> List[Dict[str, Any]]:
    ranked = sorted(records, key=lambda r: _train_objective(r))
    return ranked[:max(0, min(k, len(ranked)))]


def _build_stage3_anchor_center_from_history(
    phase1_hist: List[Dict[str, Any]],
    phase2_hist: List[Dict[str, Any]],
) -> Optional[Dict[str, float]]:
    best_stage1 = _best_record(phase1_hist)
    best_stage2 = _best_record(phase2_hist)

    if best_stage1 is None or best_stage2 is None:
        return None

    best_costs = _extract_cost_theta(best_stage1["theta"])
    best_planner = _extract_planner_theta(best_stage2["theta"])
    return _merge_theta(best_planner, best_costs)


def _pick_stage_aware_source(history: List[Dict[str, Any]], checkpoint_filtered: int) -> Optional[Dict[str, Any]]:
    ...
    if checkpoint_filtered <= STAGE1_END_FILTERED_TRIAL:
        candidates = phase1_hist
        fallbacks = []
    elif checkpoint_filtered <= STAGE2_END_FILTERED_TRIAL:
        candidates = phase2_hist
        fallbacks = [phase1_hist]
    else:
        candidates = phase3_hist
        fallbacks = [phase2_hist, phase1_hist]
...
def _build_algorithm_thinks_theta(
    history: List[Dict[str, Any]],
    checkpoint_filtered: int,
    ...
    # ---------------- stage 1 ----------------
    if checkpoint_filtered <= STAGE1_END_FILTERED_TRIAL:
        G1, _ = _build_G_L_generic(phase1_hist)
        if len(G1) == 0:
            return None, "phase1_no_G1"

        cost_center = _weighted_center_from_records(G1, COST_BOUNDS)
        theta = dict(SAFE_PLANNER)
        theta.update(cost_center)
        return theta, "phase1_G1_center"

    # ---------------- stage 2 ----------------
    if checkpoint_filtered <= STAGE2_END_FILTERED_TRIAL:
        best_stage1 = _best_record(phase1_hist)
        if best_stage1 is None:
            return None, "phase2_no_stage1"

        fixed_costs = _extract_cost_theta(best_stage1["theta"])
        G2, _ = _build_G_L_generic(phase2_hist)

        if len(G2) == 0:
            theta = _merge_theta(dict(SAFE_PLANNER), fixed_costs)
            return theta, "phase2_safe_planner_plus_best_stage1_costs"

        planner_center = _weighted_center_from_records(G2, PLANNER_BOUNDS)
        theta = _merge_theta(planner_center, fixed_costs)
        return theta, "phase2_G2_planner_center"

    # ---------------- stage 3 ----------------
    G3, _ = _build_G_L_generic(phase3_hist)
    if len(G3) > 0:
        theta = _weighted_center_from_records(G3, ALL_BOUNDS)
        return theta, "phase3_G3_joint_center"

    anchor_center = _build_stage3_anchor_center_from_history(phase1_hist, phase2_hist)
    if anchor_center is not None:
        return anchor_center, "phase3_anchor_center"

    return None, "phase3_no_center"
...
def _select_algorithm_thinks_schedule(records: List[Dict[str, Any]]) -> List[EvalTask]:
    ...
    for checkpoint_filtered in checkpoints_filtered:
        ...
        src_rec = _pick_stage_aware_source(history, checkpoint_filtered)
        if src_rec is None:
            continue

        theta_est, algo_kind = _build_algorithm_thinks_theta(history, checkpoint_filtered)
        ...
        selected.append(
            EvalTask(
                ...
                source_best_objective_so_far_train=float(best_obj_so_far),
                source_algorithm_kind=str(algo_kind),
                is_tail_padded=bool(tail_padded),
...
def _task_to_jsonable(task: EvalTask) -> Dict[str, Any]:
    return {
        ...
        "source_algorithm_kind": str(task.source_algorithm_kind),
...
def _save_plot(rows: List[Dict[str, Any]], selection_family: str, y_key: str, y_label: str) -> None:
    ...
    plt.axvline(x=STAGE1_END_FILTERED_TRIAL, linestyle="--", linewidth=1.2)
    plt.axvline(x=STAGE2_END_FILTERED_TRIAL, linestyle="--", linewidth=1.2)
...
def _print_runtime_row(
    ...
    print(
        ...
        f"{task.selection_family} | filtered={task.checkpoint_trial_filtered} | "
        f"global={task.checkpoint_trial_global} | source_phase={task.source_phase} | "
        f"kind={task.source_algorithm_kind}"
    )
