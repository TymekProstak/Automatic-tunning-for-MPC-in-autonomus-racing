...
RUN_LOG_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_RUN_LOG_PATH",
        os.path.join(SCRIPT_DIR, f"tuning_run_gp_bo_tv_{TRAIN_PROFILE_NAME}.jsonl"),
    )
)
OUTPUT_CSV_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CSV",
        os.path.join(SCRIPT_DIR, f"gpbo_tv_test_eval_{EVAL_PROFILE_NAME}.csv"),
    )
)
OUTPUT_CANDIDATES_JSON = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CANDIDATES_JSON",
        os.path.join(SCRIPT_DIR, f"gpbo_tv_test_candidates_{EVAL_PROFILE_NAME}.json"),
    )
)
PLOTS_DIR = os.path.abspath(
    os.environ.get(
        "EVAL_PLOTS_DIR",
        os.path.join(SCRIPT_DIR, f"gpbo_tv_eval_plots_{EVAL_PROFILE_NAME}"),
    )
)
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11315"))
...
def main() -> None:
    ...
    print("[INFO] stage-aware surrogate selection używa faz: phase1_cost_only_gpbo -> phase2_planner_only_gpbo -> phase3_joint_gpbo")
    print("")
