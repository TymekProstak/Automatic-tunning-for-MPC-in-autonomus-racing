...
DEFAULT_CATKIN_WS = os.path.expanduser(
    "~/Desktop/fs_control_ka_racing/TUNE_BO_GP"
)
...
# PROFILE SWITCH
# ------------------------------------------------------------
EVAL_PROFILE_NAME = os.environ.get("EVAL_PROFILE", "night_7track")
...
# ============================================================
# Manual theta from hand
# Edytuj tutaj swoje nastawy.
# ============================================================
MANUAL_THETA: Dict[str, float] = {
    # ---------------- Planner ----------------
    "velocity_planner.v_max": 17.5,
    "velocity_planner.mux_acc": 0.85,
    "velocity_planner.mux_dec": 0.85,
    "velocity_planner.muy": 1.1,

    # ---------------- Costs ----------------
    "mpc.cost.Q_y": 30.0,
    "mpc.cost.Q_psi": 60.0,
    "mpc.cost.R_ddelta": 5.0,
    "mpc.cost.R_tv" : 5.0e-6,
}

# ============================================================
# Tuned params / bounds
# Trzymam zgodnie z przykładowym evalem GP/BO.
# ============================================================
PLANNER_BOUNDS = {
...
COST_BOUNDS = {
    "mpc.cost.Q_y": (0.1, 100.0),
    "mpc.cost.Q_psi": (0.1, 100.0),
    "mpc.cost.R_ddelta": (0.1, 100.0),
}
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11331"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_manual_single_eval_{EVAL_PROFILE_NAME}")
...
# ============================================================
# Cost function
# Trzymam dokładnie w duchu Twojego przykładowego evala.
# ============================================================
COST_WEIGHTS = {
    "vs_avg_mps": -2.0,
...
def _sanitize_theta(theta: Dict[str, float]) -> Dict[str, float]:
    ...
    if not out:
        raise ValueError("No valid manual parameters remained after sanitization.")
...
def main() -> None:
    ...
    try:
        ...
        print("\n[RUN] Starting manual single-config evaluation...\n")
    ...
    print("\n[DONE] Manual single-config evaluation finished.\n")
