...
# ============================================================
# Evaluation setup / current default baseline
# ============================================================
GLOBAL_SEED = 123
...
ROS_HOME = os.path.expanduser(f"~/.ros_tune_tpe_tv_{PROFILE_NAME}_{GLOBAL_SEED}")
...
TOP10_PATH = os.path.abspath(
    os.environ.get(
        "TUNE_TOP10_PATH",
        os.path.join(SCRIPT_DIR, f"top10_tpe_tv_so_far_{PROFILE_NAME}.json"),
    )
)
SUMMARY_PATH = os.path.abspath(
    os.environ.get(
        "TUNE_SUMMARY_PATH",
        os.path.join(SCRIPT_DIR, f"tuning_summary_tpe_tv_{PROFILE_NAME}.json"),
    )
)
RUN_LOG_PATH = os.path.abspath(
    os.environ.get(
        "TUNE_RUN_LOG_PATH",
        os.path.join(SCRIPT_DIR, f"tuning_run_tpe_tv_{PROFILE_NAME}.jsonl"),
    )
)
BEST_JSON_PATH = os.path.join(
    os.path.dirname(CONTROL_PARAM_JSON),
    f"control_param_best_lti_3stage_tpe_tv_{PROFILE_NAME}.json",
)
...
def save_summary(
    ...
    payload = {
        "config": {
            ...
            "stage3_center_theta": {k: float(v) for k, v in phase3_center_theta.items()},
...
def main() -> None:
    ...
    print("[CFG] PROFILE    =", PROFILE_NAME)
    ...
    print("[CFG] Phase1 =", PHASE1_N_TRIALS, "custom-TPE trials")
    print("[CFG] Phase2 =", PHASE2_N_TRIALS, "custom-TPE trials")
    print("[CFG] Phase3 =", PHASE3_N_TRIALS, "custom-TPE trials")
    ...
    try:
        ...
        phase1_theta, phase1_obj = run_stage1_tpe(
        ...
        phase2_theta, phase2_obj = run_stage2_planner_tpe(
        ...
        print("[PHASE3] startup zawiera SAFE_PLANNER + best_costs_from_stage1")
        print("[PHASE3] oraz anchory z top plannerów stage2 i top kosztów stage1\n")

        phase3_theta, phase3_obj, phase3_center_theta = run_stage3_joint_tpe(
