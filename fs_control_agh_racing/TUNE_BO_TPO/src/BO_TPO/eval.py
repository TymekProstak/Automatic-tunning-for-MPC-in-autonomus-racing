...
# Optional overrides: phase budgets/startups (must match the tuning run log)
PHASE1_N_TRIALS = int(os.environ.get("EVAL_PHASE1_N_TRIALS", str(PHASE1_N_TRIALS)))
PHASE1_STARTUP = int(os.environ.get("EVAL_PHASE1_STARTUP", str(PHASE1_STARTUP)))

PHASE2_N_TRIALS = int(os.environ.get("EVAL_PHASE2_N_TRIALS", str(PHASE2_N_TRIALS)))
PHASE2_STARTUP = int(os.environ.get("EVAL_PHASE2_STARTUP", str(PHASE2_STARTUP)))

PHASE3_N_TRIALS = int(os.environ.get("EVAL_PHASE3_N_TRIALS", str(PHASE3_N_TRIALS)))
PHASE3_STARTUP = int(os.environ.get("EVAL_PHASE3_STARTUP", str(PHASE3_STARTUP)))
...
RUN_LOG_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_RUN_LOG_PATH",
        os.path.join(SCRIPT_DIR, f"tuning_run_tpe_tv_{TRAIN_PROFILE_NAME}.jsonl"),
    )
)
OUTPUT_CSV_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CSV",
        os.path.join(SCRIPT_DIR, f"tpe_tv_test_eval_{EVAL_PROFILE_NAME}.csv"),
    )
)
OUTPUT_CANDIDATES_JSON = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CANDIDATES_JSON",
        os.path.join(SCRIPT_DIR, f"tpe_tv_test_candidates_{EVAL_PROFILE_NAME}.json"),
    )
)
PLOTS_DIR = os.path.abspath(
    os.environ.get(
        "EVAL_PLOTS_DIR",
        os.path.join(SCRIPT_DIR, f"tpe_tv_eval_plots_{EVAL_PROFILE_NAME}"),
    )
)
...
# ============================================================
# Tuner config copied from 3-stage TPE TV tuner
# ============================================================
PHASE1_NAME = "phase1_cost_only_tpe"
PHASE2_NAME = "phase2_planner_only_tpe"
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11316"))
...
def _select_algorithm_thinks_schedule(records: List[Dict[str, Any]]) -> List[EvalTask]:
    ...
    for checkpoint_filtered in checkpoints_filtered:
        ...
        if checkpoint_filtered <= STAGE1_END_FILTERED_TRIAL:
            ...
            fixed_costs = _extract_cost_theta(src_rec["theta"])
            theta_est, score = build_tpe_local_lhs_estimate(
                G=G,
                L=L,
...
def _save_plot(rows: List[Dict[str, Any]], selection_family: str, y_key: str, y_label: str) -> None:
    ...
    plt.axvline(x=STAGE1_END_FILTERED_TRIAL, linestyle="--", linewidth=1.2, label="koniec stage 1")
    plt.axvline(x=STAGE2_END_FILTERED_TRIAL, linestyle="--", linewidth=1.2, label="koniec stage 2")
...
def main() -> None:
    ...
    print("[INFO] algorithm_thinks = stage-aware TPE local-LHS approximation")
    print("")
