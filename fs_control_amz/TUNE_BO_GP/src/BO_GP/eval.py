...
RUN_LOG_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_RUN_LOG_PATH",
        os.path.join(SCRIPT_DIR, f"tuning_run_full_coupled_gpbo_{TRAIN_PROFILE_NAME}.jsonl"),
    )
)
OUTPUT_CSV_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CSV",
        os.path.join(SCRIPT_DIR, f"full_coupled_gpbo_test_eval_{EVAL_PROFILE_NAME}.csv"),
    )
)
OUTPUT_CANDIDATES_JSON = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CANDIDATES_JSON",
        os.path.join(SCRIPT_DIR, f"full_coupled_gpbo_test_candidates_{EVAL_PROFILE_NAME}.json"),
    )
)
PLOTS_DIR = os.path.abspath(
    os.environ.get(
        "EVAL_PLOTS_DIR",
        os.path.join(SCRIPT_DIR, f"full_coupled_gpbo_eval_plots_{EVAL_PROFILE_NAME}"),
    )
)
...
# ============================================================
# Optional support for legacy virtual slack params
# ============================================================
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "1135"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_eval_full_coupled_gpbo_{EVAL_PROFILE_NAME}_{GLOBAL_SEED}")
...
def _best_so_far_from_rec(rec: Dict[str, Any]) -> float:
    ...
    return float(_train_objective(rec))
