...
# Shared budgets with uncoupled 3-stage GP-BO
# ============================================================
GLOBAL_SEED = 123
...
# ============================================================
# Safe aggressiveness / model point
#
# NOTE:
# q_sdot is formally a cost parameter, but by agreement we treat it as
# an aggression-like parameter:
# - fixed in Stage 1
# - tuned with max_vx / mux / muy in Stage 2
# ============================================================
SAFE_PLANNER = {
...
#
# Stage 1 (cost-only):
#   q_ey, Q_epsi, R_dT, R_u_ddelta_cmd, R_Mtv, Q_beta
#
# Stage 2 (planner / model aggression):
...
# Costs in log-space, except q_sdot which is explicitly linear
...
ROS_MASTER_PORT = 11319
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_tune_gpbo_full_coupled_{PROFILE_NAME}_{GLOBAL_SEED}")
...
TOP10_PATH = os.path.join(SCRIPT_DIR, f"top10_full_coupled_gpbo_{PROFILE_NAME}.json")
SUMMARY_PATH = os.path.join(SCRIPT_DIR, f"tuning_summary_full_coupled_gpbo_{PROFILE_NAME}.json")
RUN_LOG_PATH = os.path.join(SCRIPT_DIR, f"tuning_run_full_coupled_gpbo_{PROFILE_NAME}.jsonl")
BEST_JSON_PATH = os.path.join(
    os.path.dirname(CONTROL_PARAM_JSON),
    f"control_param_best_full_coupled_3stage_gpbo_{PROFILE_NAME}.json",
)
...
def build_phase2_startup_planner_only(
    ...
    """
    Stage 2:
    - Stage 1 costs are frozen
    - exact SAFE_PLANNER
    - local cloud around SAFE_PLANNER
    - remaining points = wide LHS over [max_vx, mux, muy, q_sdot]

    NOTE:
    q_sdot is grouped here with aggression-like variables on purpose.
    """
    rng = random.Random(seed)
    out: List[Dict[str, float]] = []
...
# q_ey, Q_epsi, R_dT, R_u_ddelta_cmd, R_Mtv, Q_beta
# fixed: max_vx, mux, muy, q_sdot=0.1
# ============================================================
def run_stage1_gpbo(
    n_trials: int,
    startup_trials: int,
...
# ============================================================
# Stage 2: planner / aggression GP-BO
# max_vx, mux, muy, q_sdot
# fixed: best Stage 1 costs
# ============================================================
def run_stage2_gpbo(
    n_trials: int,
    startup_trials: int,
...
def main() -> None:
    ...
    print("[CFG] Phase1 =", PHASE1_N_TRIALS, "cost-only GP-BO trials")
    print("[CFG] Phase2 =", PHASE2_N_TRIALS, "planner/aggr GP-BO trials")
    print("[CFG] Phase3 =", PHASE3_N_TRIALS, "joint GP-BO trials")
    ...
    print("[CFG] q_sdot jest liniowy i strojony dopiero od Stage 2.")
    print("")
    ...
    try:
        ...
        phase3_theta, phase3_J, stage3_center_theta = run_stage3_gpbo(
