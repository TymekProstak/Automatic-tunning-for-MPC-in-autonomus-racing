...
RUN_LOG_CANDIDATES = [
    os.path.join(SCRIPT_DIR, f"tuning_run_full_coupled_unstaged_tpe_{TRAIN_PROFILE_NAME}.jsonl"),
    os.path.join(SCRIPT_DIR, f"tuning_run_unstaged_full_coupled_tpe_{TRAIN_PROFILE_NAME}.jsonl"),
    os.path.join(SCRIPT_DIR, f"tuning_run_coupled_unstaged_tpe_{TRAIN_PROFILE_NAME}.jsonl"),
    os.path.join(SCRIPT_DIR, f"tuning_run_unstaged_tpe_{TRAIN_PROFILE_NAME}.jsonl"),
    os.path.join(SCRIPT_DIR, "tuning_run_full_coupled_unstaged_tpe.jsonl"),
    os.path.join(SCRIPT_DIR, "tuning_run_unstaged_full_coupled_tpe.jsonl"),
    os.path.join(SCRIPT_DIR, "tuning_run_coupled_unstaged_tpe.jsonl"),
    os.path.join(SCRIPT_DIR, "tuning_run_unstaged_tpe.jsonl"),
]
...
OUTPUT_CSV_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CSV",
        os.path.join(SCRIPT_DIR, f"tpe_full_coupled_unstaged_test_eval_{EVAL_PROFILE_NAME}.csv"),
    )
)
...
OUTPUT_CANDIDATES_JSON = os.path.abspath(
    os.environ.get(
        ...
        os.path.join(SCRIPT_DIR, f"tpe_full_coupled_unstaged_test_candidates_{EVAL_PROFILE_NAME}.json"),
    )
)
...
PLOTS_DIR = os.path.abspath(
    os.environ.get(
        "EVAL_PLOTS_DIR",
        os.path.join(SCRIPT_DIR, f"tpe_full_coupled_unstaged_eval_plots_{EVAL_PROFILE_NAME}"),
    )
)
...
# ============================================================
# Unstaged coupled TPE config
# ============================================================
UNSTAGED_PHASE_NAME = os.environ.get("EVAL_UNSTAGED_PHASE_NAME", "unstaged_full_coupled_tpe").strip()
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11832"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_eval_full_coupled_unstaged_tpe_{EVAL_PROFILE_NAME}_{GLOBAL_SEED}")
...
def _is_startup_record(rec: Dict[str, Any]) -> bool:
    ...
    return int(rec.get("_global_trial", 10**18)) <= UNSTAGED_STARTUP
...
def main() -> None:
    ...
    print("[INFO] algorithm_thinks = dense local LHS over current global G/L.")
    print("")
