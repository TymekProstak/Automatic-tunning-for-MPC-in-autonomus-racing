...
# Config
# ============================================================
WS="${EVAL_WS:-$HOME/Desktop/fs_control_amz/NO_STAGE/TUNE_BO_TPO}"

# Python eval script
PY_SCRIPT_REL="${EVAL_PY_SCRIPT_REL:-src/BO_TPO/eval.py}"

# Input tuning log JSONL
RUN_LOG_JSONL="${EVAL_RUN_LOG_PATH:-$WS/src/BO_TPO/tuning_run_full_coupled_tpe_unstaged_night_4track.jsonl}"
...
RUN_LOG="$RUN_LOG_DIR/eval_tpe_test_tracks_full_coupled_${STAMP}.log"
...
# Najważniejsze: tego oczekuje eval.py
export EVAL_RUN_LOG_PATH="$RUN_LOG_JSONL"

# Zostawiam też starą nazwę pomocniczo, jeśli gdzieś indziej jej używasz
export TEST_TUNING_RUN_JSONL="$RUN_LOG_JSONL"

# ============================================================
# 7) Run test pipeline
# ============================================================
echo "[RUN] Starting TPE Full Coupled test-eval pipeline..."
