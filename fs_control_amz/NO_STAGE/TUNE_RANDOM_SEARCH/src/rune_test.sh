...
# Config
# ============================================================
WS="${EVAL_WS:-$HOME/Desktop/fs_control_amz/NO_STAGE/TUNE_RANDOM_SEARCH}"

# Ścieżka do skryptu ewaluacyjnego w Pythonie
PY_SCRIPT_REL="${EVAL_PY_SCRIPT_REL:-src/random_search/eval.py}"

# Ścieżka do pliku z wynikami tuningu, z którego skrypt weźmie kandydatów
RUN_LOG_JSONL="${EVAL_RUN_LOG_PATH:-$WS/src/random_search/tuning_run_full_coupled_unstaged_random_search_night_4track.jsonl}"
...
RUNNER_LOG="$RUN_LOG_DIR/eval_random_test_tracks_full_coupled_${STAMP}.log"
...
# Możesz to nadpisać przed uruchomieniem w konsoli (eksportując zmienne):
# export EVAL_OUTPUT_CSV="$WS/src/random_search/random_search_test_eval_full_coupled.csv"
# export EVAL_BEST_SO_FAR_EVERY=10
# export EVAL_SURROGATE_EVERY=12
# export EVAL_TOPK=3
# export EVAL_SIM_TIME_S=60
# export EVAL_ROS_MASTER_PORT=11334
