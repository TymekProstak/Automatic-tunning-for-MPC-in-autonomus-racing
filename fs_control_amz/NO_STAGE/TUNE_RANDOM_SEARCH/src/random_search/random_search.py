...
# ============================================================
# Single-phase crash multiplier
# ============================================================
UNSTAGED_CRITICAL_CRASH_MULTIPLIER = float(
    os.environ.get("TUNE_CRITICAL_CRASH_MULTIPLIER", "1.0")
)
...
ROS_MASTER_PORT = int(os.environ.get("TUNE_ROS_MASTER_PORT", "11422"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_tune_random_full_coupled_unstaged_{PROFILE_NAME}_{GLOBAL_SEED}")
...
TOP10_PATH = os.path.join(SCRIPT_DIR, f"top10_full_coupled_unstaged_random_search_{PROFILE_NAME}.json")
SUMMARY_PATH = os.path.join(SCRIPT_DIR, f"tuning_summary_full_coupled_unstaged_random_search_{PROFILE_NAME}.json")
RUN_LOG_PATH = os.path.join(SCRIPT_DIR, f"tuning_run_full_coupled_unstaged_random_search_{PROFILE_NAME}.jsonl")
BEST_JSON_PATH = os.path.join(
    os.path.dirname(CONTROL_PARAM_JSON),
    f"control_param_best_full_coupled_unstaged_random_search_{PROFILE_NAME}.json",
)
...
def _make_cost_mid_theta() -> Dict[str, float]:
    ...
    for k, (lo, hi) in COST_BOUNDS.items():
        out[k] = float(math.sqrt(lo * hi)) if k in LOG_PARAMS else float(0.5 * (lo + hi))
    return out
...
def run_one_episode(
    ...
    critical_crash_multiplier: float = UNSTAGED_CRITICAL_CRASH_MULTIPLIER,
) -> EpisodeResult:
...
def build_unstaged_startup_joint(
    ...
    def _push(theta: Dict[str, float]) -> None:
        key = _theta_key(theta)
        if key not in seen:
            out.append(dict(theta))
            seen.add(key)

    conservative = _merge_theta(dict(SAFE_PLANNER), _make_cost_mid_theta())
    _push(conservative)

    remaining = max(0, n_points - len(out))
    if remaining > 0:
        lhs = generate_lhs_trials(seed, remaining, ALL_BOUNDS)
        for th in lhs:
            _push(th)
            if len(out) >= n_points:
                break
    ...
    while len(out) < n_points:
        _push(sample_theta_global(rng, ALL_BOUNDS))
...
# ============================================================
# Single unstaged phase
# ============================================================
def run_unstaged_random_search(
    n_trials: int,
    startup_trials: int,
    critical_crash_multiplier: float,
) -> Tuple[Dict[str, float], float, Dict[str, float], int, int]:
    ...
    center_theta = dict(startup[0]) if startup else _merge_theta(dict(SAFE_PLANNER), _make_cost_mid_theta())
    ...
    for i in range(min(n_trials, len(startup))):
        ...
        _register_result(
            ...
            info,
            extra_log={
                "proposal_kind": "unstaged_startup_joint",
                "is_global": False,
            },
        )
    ...
    for i in range(len(ALL_RESULTS), n_trials):
        ...
        if do_global or len(ALL_RESULTS) == 0:
            ...
            G, _ = build_G_L_from_results(ALL_RESULTS)
            center_rec = _weighted_choice_record(rng, G)
            center_theta = {k: float(v) for k, v in center_rec["theta"].items()}
    ...
    G_fin, L_fin = build_G_L_from_results(ALL_RESULTS)
    return best_theta, best_J, center_theta, len(G_fin), len(L_fin)
...
def save_summary(
    ...
    payload = {
        "config": {
            ...
            "common_architecture": UNSTAGED_PHASE_NAME,
            "slack_tuning": False,
            "startup_center_theta": {k: float(v) for k, v in center_theta.items()},
...
def main() -> None:
    ...
    print(f"[CFG] CRITICAL_CRASH_MULTIPLIER = {UNSTAGED_CRITICAL_CRASH_MULTIPLIER}")
    print(f"[CFG] UCB_WEIGHT = {UCB_WEIGHT}")
    print(f"[CFG] UNSTAGED_GLOBAL_RANDOM_FRACTION   = {UNSTAGED_GLOBAL_RANDOM_FRACTION}")
    print(f"[CFG] UNSTAGED_PLANNER_SIGMA_INFLATE    = {UNSTAGED_PLANNER_SIGMA_INFLATE}")
    print(f"[CFG] UNSTAGED_PLANNER_SIGMA_MIN_FRAC   = {UNSTAGED_PLANNER_SIGMA_MIN_FRAC}")
    print(f"[CFG] UNSTAGED_PLANNER_SIGMA_MAX_FRAC   = {UNSTAGED_PLANNER_SIGMA_MAX_FRAC}")
    print(f"[CFG] UNSTAGED_COST_SIGMA_INFLATE       = {UNSTAGED_COST_SIGMA_INFLATE}")
    print(f"[CFG] UNSTAGED_COST_LOG_SIGMA_MIN_FRAC  = {UNSTAGED_COST_LOG_SIGMA_MIN_FRAC}")
    print(f"[CFG] UNSTAGED_COST_LOG_SIGMA_MAX_FRAC  = {UNSTAGED_COST_LOG_SIGMA_MAX_FRAC}")
    ...
    try:
        print("\n================ UNSTAGED FULL-COUPLED RANDOM SEARCH ================\n")
        print("[UNSTAGED] one common joint search over planner + costs")
        print("[UNSTAGED] startup = conservative anchor + wide joint LHS")
        print("[UNSTAGED] search = mix global / local around good-set points")
        print("[UNSTAGED] no separate stage1 / stage2 / stage3")
        print("[UNSTAGED] slack penalties frozen from base JSON\n")

        best_theta, best_J, center_theta, G_size, L_size = run_unstaged_random_search(
            n_trials=UNSTAGED_N_TRIALS,
            startup_trials=UNSTAGED_STARTUP,
            critical_crash_multiplier=UNSTAGED_CRITICAL_CRASH_MULTIPLIER,
        )

        print("[UNSTAGED] final G size =", G_size)
        print("[UNSTAGED] final L size =", L_size)
