...
# ============================================================
# Profile / exact coupled unstaged budget
# ============================================================
...
if PROFILE_NAME == "smoke_1track":
    ...
    UNSTAGED_N_TRIALS = PHASE1_N_TRIALS + PHASE2_N_TRIALS + PHASE3_N_TRIALS  # 224
    UNSTAGED_LAMBDA = 8
    UNSTAGED_MAXITER = 28
...
# Costs are log-space; q_sdot remains linear.
...
UNSTAGED_PLANNER_STD_FRAC_MU = 0.30
UNSTAGED_PLANNER_STD_FRAC_QSDOT = 0.30
...
ROS_MASTER_PORT = int(os.environ.get("TUNE_ROS_MASTER_PORT", "11421"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_tune_full_coupled_cma_unstaged_{PROFILE_NAME}_{GLOBAL_SEED}")
...
TOP10_PATH = os.path.join(SCRIPT_DIR, f"top10_full_coupled_unstaged_cma_{PROFILE_NAME}.json")
SUMMARY_PATH = os.path.join(SCRIPT_DIR, f"tuning_summary_full_coupled_unstaged_cma_{PROFILE_NAME}.json")
RUN_LOG_PATH = os.path.join(SCRIPT_DIR, f"tuning_run_full_coupled_unstaged_cma_{PROFILE_NAME}.jsonl")
DIST_LOG_PATH = os.path.join(SCRIPT_DIR, f"cma_distribution_log_full_coupled_unstaged_cma_{PROFILE_NAME}.jsonl")
BEST_JSON_PATH = os.path.join(
    os.path.dirname(CONTROL_PARAM_JSON),
    f"control_param_best_full_coupled_unstaged_cma_{PROFILE_NAME}.json",
)
...
def _build_unstaged_transform() -> Tuple[SearchTransform, np.ndarray]:
    ...
    for k in PLANNER_KEYS:
        ...
        if k == "mpc.bounds.max_vx":
            frac = UNSTAGED_PLANNER_STD_FRAC_VMAX
        elif k == "mpc.cost.q_sdot":
            frac = UNSTAGED_PLANNER_STD_FRAC_QSDOT
        else:
            frac = UNSTAGED_PLANNER_STD_FRAC_MU
    ...
    while (not es.stop()) and generation < maxiter:
        ...
        for i, yi in enumerate(Y):
            ...
            print(
                ...
                f"obj={objective_cost:.6g} "
                f"mean={info.get('mean_cost', float('nan')):.6g} "
        ...
        final_dist["generation_best_cost"] = float(gen_best_obj)
        final_dist["global_best_cost_inside_phase"] = float(best_obj)
...
def save_summary(
    ...
    payload = {
        "config": {
            ...
            "unstaged_planner_std_frac_qsdot": UNSTAGED_PLANNER_STD_FRAC_QSDOT,
...
def main() -> None:
    ...
    print("[CFG] Joint CMA from the beginning over all tuned params")
    print("[CFG] q_sdot is linear; true costs are log-space")
    ...
    try:
        print("\n================ UNSTAGED JOINT COUPLED CMA-ES ================\n")
        print("[UNSTAGED] free: q_ey, Q_epsi, R_dT, R_u_ddelta_cmd, R_Mtv, Q_beta, max_vx, mux, muy, q_sdot")
        print("[UNSTAGED] no staged freezing, no TPE, no KDE")
        print("[UNSTAGED] one joint CMA-ES distribution from the beginning\n")
