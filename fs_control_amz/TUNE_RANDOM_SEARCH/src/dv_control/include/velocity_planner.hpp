...
struct VelocityPlannerResult
{
    ...
    // !!! tylko deklaracja, const !!!
    double v_on_spline_interpolate(const double s_query) const;
};
...
// helper: mapuje s_wrapped na “unwrapped” blisko s_ref (tak jak w MPCCInterface)
static inline double unwrap_relative_s(double s_wrapped, double s_ref, double L)
{
    if (L <= 1e-12) return s_wrapped;
    const double k = std::floor(s_ref / L);
    double s = k * L + s_wrapped;
    while (s - s_ref >  0.5 * L) s -= L;
    while (s - s_ref <= -0.5 * L) s += L;
    return s;
}

inline double VelocityPlannerResult::v_on_spline_interpolate(const double s_query) const
{
    if (!valid) return 0.0;
    if (N < 2) return 0.0;
    if (velocity_ref.size() != N || s_ref.size() != N) return 0.0;

    const double L = track_length;

    // obsłuż zarówno s wrapped jak i unwrapped
    double s_wrapped = s_query;
    if (L > 1e-12) s_wrapped = wrapS(s_query, L);

    // mapuj na unwrapped w okolicy s_ref(0)
    const double s0 = s_ref(0);
    const double s = (L > 1e-12) ? unwrap_relative_s(s_wrapped, s0, L) : s_query;

    // clamp do zakresu profilu
    const double s_min = s_ref(0);
    const double s_max = s_ref(N - 1);
    const double sq = std::clamp(s, s_min, s_max);

    // binary search po s_ref (monotoniczne)
    int lo = 0, hi = N - 1;
    while (hi - lo > 1) {
        const int mid = (lo + hi) / 2;
        if (s_ref(mid) <= sq) lo = mid;
        else hi = mid;
    }

    const double sL = s_ref(lo);
    const double sR = s_ref(lo + 1);
    const double vL = velocity_ref(lo);
    const double vR = velocity_ref(lo + 1);

    const double denom = std::max(1e-9, sR - sL);
    const double a = std::clamp((sq - sL) / denom, 0.0, 1.0);

    return (1.0 - a) * vL + a * vR;
}
...
{
    const double safety = P.get("vel_planner_safety_factor");
    ...
    double s0_exact)
{
    ...
    // OKNO: 60 m do przodu (jeśli tor krótszy, to max L)
    const double S_plan = std::min(60.0, L);
    if (S_plan <= ds_geom) {
    ...
    for (int i = 0; i < K; ++i) {
        ...
        const double s_wrapped = wrapS(s_unwrap, L);

        // Ja zapisuję s_ref jako unwrapped, monotoniczne
        out.s_ref(i) = s_unwrap;

        const auto pr = profile_atS(prof, s_wrapped, L);

        out.curvature(i)         = spline_closed.getCurvature(s_wrapped);
        out.velocity_ref(i)      = pr.v_ref;
        out.acceleration_ref(i)  = pr.a_ref;

        Vec2 p = spline_closed.eval(s_wrapped);
        out.X_ref(i) = p.x;
        out.Y_ref(i) = p.y;

        // Ja robię predykcję s w czasie (unwrapped)
        s_unwrap += pr.v_ref * mpc_dt + 0.5 * pr.a_ref * mpc_dt * mpc_dt;
    }
