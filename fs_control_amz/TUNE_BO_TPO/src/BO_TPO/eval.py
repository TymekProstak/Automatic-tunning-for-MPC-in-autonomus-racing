...
RUN_LOG_CANDIDATES = [
    os.path.join(SCRIPT_DIR, f"tuning_run_full_coupled_tpe_{TRAIN_PROFILE_NAME}.jsonl"),
    os.path.join(SCRIPT_DIR, f"tuning_run_coupled_tpe_{TRAIN_PROFILE_NAME}.jsonl"),
    os.path.join(SCRIPT_DIR, "tuning_run_full_coupled_tpe.jsonl"),
    os.path.join(SCRIPT_DIR, "tuning_run_coupled_tpe.jsonl"),
]

RUN_LOG_PATH = os.path.abspath(
    os.environ.get("EVAL_RUN_LOG_PATH", _pick_first_existing(RUN_LOG_CANDIDATES))
)

OUTPUT_CSV_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_OUTPUT_CSV",
        os.path.join(SCRIPT_DIR, f"tpe_full_coupled_test_eval_{EVAL_PROFILE_NAME}.csv"),
    )
)
...
OUTPUT_CANDIDATES_JSON = os.path.abspath(
    os.environ.get(
        ...
        os.path.join(SCRIPT_DIR, f"tpe_full_coupled_test_candidates_{EVAL_PROFILE_NAME}.json"),
    )
)
...
PLOTS_DIR = os.path.abspath(
    os.environ.get(
        "EVAL_PLOTS_DIR",
        os.path.join(SCRIPT_DIR, f"tpe_full_coupled_eval_plots_{EVAL_PROFILE_NAME}"),
    )
)
...
# ============================================================
# Coupled AMZ 3-stage TPE config copied from tuner baseline
# ============================================================
PHASE1_NAME = "phase1_cost_only_tpe"
PHASE2_NAME = "phase2_planner_qsdot_tpe"
PHASE3_NAME = "phase3_joint_tpe"
...
TOTAL_EFFECTIVE_TRIALS = (
    PHASE1_EFFECTIVE_TRIALS + PHASE2_EFFECTIVE_TRIALS + PHASE3_EFFECTIVE_TRIALS
)
...
Q_SDOT_KEY = "mpc.cost.q_sdot"
SAFE_Q_SDOT = float(os.environ.get("EVAL_SAFE_Q_SDOT", "0.1"))
...
PHASE1_COST_BOUNDS: Dict[str, Tuple[float, float]] = {}
PHASE1_COST_BOUNDS.update(BASE_COST_BOUNDS)

PHASE2_AGGR_BOUNDS: Dict[str, Tuple[float, float]] = {}
PHASE2_AGGR_BOUNDS.update(PLANNER_BOUNDS)
PHASE2_AGGR_BOUNDS.update(Q_SDOT_BOUNDS)

ALL_BOUNDS: Dict[str, Tuple[float, float]] = {}
ALL_BOUNDS.update(PLANNER_BOUNDS)
ALL_BOUNDS.update(BASE_COST_BOUNDS)
ALL_BOUNDS.update(Q_SDOT_BOUNDS)

LOG_PARAMS = set(BASE_COST_BOUNDS.keys())
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11732"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_eval_full_coupled_tpe_{EVAL_PROFILE_NAME}_{GLOBAL_SEED}")
...
def _extract_base_cost_theta(theta: Dict[str, float]) -> Dict[str, float]:
    return {k: float(theta[k]) for k in BASE_COST_BOUNDS.keys()}
...
# ============================================================
# Run-log loading / filtered trial helpers
# ============================================================
...
def evaluate_theta_on_test_tracks(
    ...
    var_cost = sum(
        (float(r.cost) - mean_cost) * (float(r.cost) - mean_cost)
...
def _select_algorithm_thinks_schedule(records: List[Dict[str, Any]]) -> List[EvalTask]:
    ...
    for checkpoint_filtered in checkpoints_filtered:
        ...
        phase1_hist = [r for r in history if str(r.get("phase", "")) == PHASE1_NAME]
        phase2_hist = [r for r in history if str(r.get("phase", "")) == PHASE2_NAME]
        phase3_hist = [r for r in history if str(r.get("phase", "")) == PHASE3_NAME]

        # --------------------------------------------------------
        # Stage 1:
        # dense local LHS over phase1 G/L in cost space
        # fixed = SAFE_PLANNER + SAFE_Q_SDOT
        # --------------------------------------------------------
        if checkpoint_filtered <= STAGE1_END_FILTERED_TRIAL:
            if not phase1_hist:
                continue

            src_rec = phase1_hist[-1]
            G, L = _build_G_L_generic(phase1_hist)
            if len(G) == 0:
                continue

            fixed_phase1 = dict(SAFE_PLANNER)
            fixed_phase1[Q_SDOT_KEY] = float(SAFE_Q_SDOT)

            theta_est, score = build_tpe_local_lhs_estimate(
                G=G,
                L=L,
                bounds=PHASE1_COST_BOUNDS,
                fixed_params=fixed_phase1,
                seed=GLOBAL_SEED + 800000 + checkpoint_filtered,
            )

            label = f"algorithm_thinks_stage1_t{checkpoint_filtered:03d}"

        # --------------------------------------------------------
        # Stage 2:
        # dense local LHS over phase2 G/L in planner+q_sdot space
        # fixed = BEST phase1 base costs
        # --------------------------------------------------------
        elif checkpoint_filtered <= STAGE2_END_FILTERED_TRIAL:
            if not phase2_hist or not phase1_hist:
                continue

            src_rec = phase2_hist[-1]

            best_phase1_rec = _best_record(phase1_hist)
            if best_phase1_rec is None:
                continue

            fixed_base_costs = _extract_base_cost_theta(best_phase1_rec["theta"])

            G, L = _build_G_L_generic(phase2_hist)
            if len(G) == 0:
                continue
            ...
            theta_est, score = build_tpe_local_lhs_estimate(
                G=G,
                L=L,
                bounds=PHASE2_AGGR_BOUNDS,
                fixed_params=fixed_base_costs,
                seed=GLOBAL_SEED + 900000 + checkpoint_filtered,
            )

            label = f"algorithm_thinks_stage2_t{checkpoint_filtered:03d}"

        # --------------------------------------------------------
        # Stage 3:
        # dense local LHS over phase3 G/L in full space
        # fixed = none
        # --------------------------------------------------------
        else:
            if not phase3_hist:
            ...
            src_rec = phase3_hist[-1]
            G, L = _build_G_L_generic(phase3_hist)
            if len(G) == 0:
                continue
            ...
            label = f"algorithm_thinks_stage3_t{checkpoint_filtered:03d}"
...
def _save_plot(rows: List[Dict[str, Any]], selection_family: str, y_key: str, y_label: str) -> None:
    ...
    plt.axvline(
        x=STAGE1_END_FILTERED_TRIAL,
        linestyle="--",
        linewidth=1.2,
        label="koniec stage 1",
    )
    plt.axvline(
        x=STAGE2_END_FILTERED_TRIAL,
        linestyle="--",
        linewidth=1.2,
        label="koniec stage 2",
    )
...
def main() -> None:
    ...
    original_control_json = copy.deepcopy(BASE_CONTROL_TEMPLATE)
    ...
    print("[CFG] STAGE1_END_FILTERED     =", STAGE1_END_FILTERED_TRIAL)
    print("[CFG] STAGE2_END_FILTERED     =", STAGE2_END_FILTERED_TRIAL)
    ...
    print("[CFG] SAFE_PLANNER            =", SAFE_PLANNER)
    print("[CFG] SAFE_Q_SDOT             =", SAFE_Q_SDOT)
    print("[CFG] PHASE1_NAME             =", PHASE1_NAME)
    print("[CFG] PHASE2_NAME             =", PHASE2_NAME)
    print("[CFG] PHASE3_NAME             =", PHASE3_NAME)
    print("")

    print(f"[INFO] Loaded {len(records)} tuning records from run log jsonl.")
    print("[INFO] best_so_far biorę z pełnego run loga, nie z top10 json.")
    print("[INFO] algorithm_thinks = dense local LHS over current-stage G/L.")
    print("[INFO] stage2 algorithm_thinks: G/L only from phase2, fixed costs = best phase1.")
    print("")
