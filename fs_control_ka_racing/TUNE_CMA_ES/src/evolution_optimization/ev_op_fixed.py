...
# ============================================================
# Stage 1: stroję tylko koszty przy bezpiecznym plannerze.
STAGE1_COST_STD_FRAC = 0.30

# Stage 2: planner-only, szeroko wokół SAFE_PLANNER.
if PROFILE_NAME == "smoke_1track":
...
# Stage 3: joint, planner lekko blenduję z SAFE_PLANNER, ale słabo,
# żeby nie betonować środka.
if PROFILE_NAME == "smoke_1track":
...
ROS_HOME = os.path.expanduser(f"~/.ros_tune_cma_{PROFILE_NAME}_{GLOBAL_SEED}")
...
DIST_LOG_PATH = os.path.join(SCRIPT_DIR, f"cma_distribution_log_{PROFILE_NAME}.jsonl")
BEST_JSON_PATH = os.path.join(
    os.path.dirname(CONTROL_PARAM_JSON),
    f"control_param_best_lti_3stage_cma_{PROFILE_NAME}.json"
)
...
def main() -> None:
    ...
    try:
        ...
        print("\n================ PHASE 2: PLANNER ONLY @ FIXED BEST COSTS (CMA-ES) ================\n")
