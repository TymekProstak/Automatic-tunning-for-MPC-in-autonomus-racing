...
void Controller::pathCallback(const dv_interfaces::LtoResult &msg)
{
    ...
    X_last_from_pp = Eigen::Map<const Eigen::VectorXd>(msg.track_x.data(), static_cast<int>(N));
    Y_last_from_pp = Eigen::Map<const Eigen::VectorXd>(msg.track_y.data(), static_cast<int>(N));
...
void Controller::odometryCallback(const nav_msgs::Odometry &msg)
{
    ...
    // 1) getCurrentState
    // ---------------------------
    const auto t_get0 = Clock::now();
    getCurrentState(msg);
    convert_state_to_mpc_state();
    const auto t_get1 = Clock::now();
    ...
    // sekcyjne czasy (ms) — żeby ładnie zsumować do statystyk
    ...
    if (ref_path.valid)
    {
        if (current_state.vx > 2.0)
        {
            ...
            dv_interfaces::MPCDebug debug_msg;
            {
                ...
                debug_msg.v_s_current  = current_state.vx * std::cos(mpc_state.epsi) - current_state.vy * std::sin(mpc_state.epsi);
                ...
                debug_msg.ay_target       = current_state.vx * current_state.vx * ref_path.curvature(0);
                debug_msg.mpc_yaw_rate_from_curvature = current_state.vx * ref_path.curvature(0);
    ...
    g_cb.getState.add(ms(t_get0, t_get1));
    g_cb.planner.add(ms(t_plan0, t_plan1));
    g_cb.convert.add(ms_convert);
    g_cb.mpc.add(ms_mpc);
...
double Controller::acc_to_throttle_percentage(double a_desired)
{
    ...
    double F_required = vehicle_mass * a_desired + drag_force
                       - vehicle_mass * current_state.yaw_rate * current_state.vy ;
                    //   + drag_force;
  //  std::cout << "F_required: " << F_required << " N" << std::endl;
    double torque_required = F_required * wheel_radius;

   // std::cout << "torque_required: " << torque_required << " Nm" << std::endl;
...
void Controller::convert_state_to_mpc_state()
{
    ...
    // mpc_state.delta, mpc_state.d_delta, mpc_state.delta_request are updated in ZOH_for_steering
}
