...
if EVAL_PROFILE_NAME == "smoke_3track":
    ...
    # odpowiada tunerowi unstaged dla smoke_1track
    DEFAULT_RUN_LOG_NAME = "tuning_run_unstaged_cma_smoke_1track.jsonl"
    DEFAULT_DIST_LOG_NAME = "cma_distribution_log_unstaged_cma_smoke_1track.jsonl"
    DEFAULT_SUMMARY_NAME = "tuning_summary_unstaged_cma_smoke_1track.json"

    DEFAULT_OUTPUT_CANDIDATES_NAME = "cma_unstaged_test_candidates_smoke3track.json"
    DEFAULT_OUTPUT_CSV_NAME = "cma_unstaged_test_results_smoke3track.csv"
    DEFAULT_PLOTS_DIR_NAME = "cma_unstaged_eval_plots_smoke3track"
    ...
    # odpowiada tunerowi unstaged dla night_4track
    DEFAULT_RUN_LOG_NAME = "tuning_run_unstaged_cma_night_4track.jsonl"
    DEFAULT_DIST_LOG_NAME = "cma_distribution_log_unstaged_cma_night_4track.jsonl"
    DEFAULT_SUMMARY_NAME = "tuning_summary_unstaged_cma_night_4track.json"

    DEFAULT_OUTPUT_CANDIDATES_NAME = "cma_unstaged_test_candidates_night7track.json"
    DEFAULT_OUTPUT_CSV_NAME = "cma_unstaged_test_results_night7track.csv"
    DEFAULT_PLOTS_DIR_NAME = "cma_unstaged_eval_plots_night7track"
...
UNSTAGED_STARTUP_TRIALS = 0
UNSTAGED_EFFECTIVE_TRIALS = UNSTAGED_N_TRIALS
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11413"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_test_eval_unstaged_cma_{EVAL_PROFILE_NAME}")
...
def _distribution_full_theta(rec: Dict[str, Any]) -> Dict[str, float]:
    ...
    if "distribution_center_theta" in rec:
        return {k: float(v) for k, v in rec["distribution_center_theta"].items()}
    return {}


def _build_unstaged_dist_records(
    ...
    for rec in dist_log:
        ...
        tmp = dict(rec)
        tmp["global_trial_end"] = int(generation * lambda_)
...
def _build_initial_distribution_record(summary: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    if summary is None:
        return None

    try:
        dist = summary["stage_distributions"]["unstaged_initial_distribution"]
    ...
    return {
        ...
        "source_kind": "unstaged_initial_distribution",
    }
...
def _select_best_so_far_schedule(run_log: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    run_log = sorted(run_log, key=lambda r: int(r["candidate_id"]))
    ...
    max_available_global = max(int(r["candidate_id"]) for r in run_log)

    for checkpoint_filtered in checkpoints_filtered:
        checkpoint_global = int(checkpoint_filtered)
        effective_global = min(checkpoint_global, max_available_global)

        prefix = [r for r in run_log if int(r["candidate_id"]) <= effective_global]
        if not prefix:
            continue
...
def _select_distribution_center_schedule(
    ...
    dist_records = _build_unstaged_dist_records(
        dist_log=dist_log,
        lambda_=UNSTAGED_LAMBDA,
    )

    init_rec = _build_initial_distribution_record(summary)
    if init_rec is not None and not any(int(r.get("generation", -1)) == 0 for r in dist_records):
        dist_records = [init_rec] + dist_records
    ...
    max_global_end = max(int(r["global_trial_end"]) for r in dist_records)

    for checkpoint_filtered in checkpoints_filtered:
        checkpoint_global = int(checkpoint_filtered)

        available = [r for r in dist_records if int(r["global_trial_end"]) <= checkpoint_global]
        if not available:
            rec = dist_records[0]
            is_tail_padded = True
        else:
            rec = available[-1]
            is_tail_padded = checkpoint_global > max_global_end
        ...
        selected.append(
            {
                ...
                "source_kind": str(rec.get("source_kind", "generation_distribution")),
...
def main() -> None:
    ...
    try:
        for idx, cand in enumerate(all_candidates):
            print(
                ...
                f"| checkpoint_trial={cand['checkpoint_trial_filtered']}"
            )
            ...
            cache_note = " | cache=1" if test_info.get("from_cache", False) else ""
            ...
            print(
                ...
                f"tail_padded={row['is_tail_padded']}{cache_note}"
            )
            print("")
    ...
    print("Unique evaluated thetas:", len(EVAL_CACHE))
    print("Total test episodes done:", EPISODES_DONE)
