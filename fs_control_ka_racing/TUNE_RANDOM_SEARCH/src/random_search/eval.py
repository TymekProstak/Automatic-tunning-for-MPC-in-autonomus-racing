...
# ============================================================
# Profile / stage sizes (must match the tuner)
# ============================================================
...
PHASE1_END_TRIAL = PHASE1_N_TRIALS
PHASE2_END_TRIAL = PHASE1_N_TRIALS + PHASE2_N_TRIALS
PHASE3_END_TRIAL = TOTAL_TRIALS

PHASE_NAMES = [
    "phase1_cost_only_random",
    "phase2_planner_only_random",
    "phase3_joint_random",
]
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11314"))
...
RUN_LOG_PATH = os.path.abspath(
    os.environ.get(
        "EVAL_RUN_LOG_PATH",
        os.path.join(SCRIPT_DIR, f"tuning_run_random_search_{PROFILE_NAME}.jsonl"),
    )
)
SELECTED_CANDIDATES_JSON = os.path.abspath(
    os.environ.get(
        "EVAL_SELECTED_CANDIDATES_JSON",
        os.path.join(SCRIPT_DIR, f"random_search_eval_candidates_{PROFILE_NAME}.json"),
    )
)
TEST_RESULTS_CSV = os.path.abspath(
    os.environ.get(
        "EVAL_RESULTS_CSV",
        os.path.join(SCRIPT_DIR, f"random_search_eval_results_{PROFILE_NAME}.csv"),
    )
)
PLOTS_DIR = os.path.abspath(
    os.environ.get(
        "EVAL_PLOTS_DIR",
        os.path.join(SCRIPT_DIR, f"random_search_eval_plots_{PROFILE_NAME}"),
    )
)

# ============================================================
# Cost function (must match the tuner)
# ============================================================
COST_WEIGHTS = {
    "vs_avg_mps": -2.0,
...
def _train_objective(rec: Dict[str, Any]) -> float:
    # In this random-search tuner the optimized quantity is stored under "mean_cost",
    # but that value is already the robust objective returned by evaluate_theta.
...
# Candidate selection
# ============================================================
def _phase_records(run_log: List[Dict[str, Any]], phase_name: str) -> List[Dict[str, Any]]:
    recs = [r for r in run_log if str(r.get("phase", "")) == phase_name]
    recs.sort(key=lambda r: int(r["candidate_id"]))
    return recs
...
def _select_global_best_so_far(run_log: List[Dict[str, Any]]) -> List[EvalTask]:
    ...
    for checkpoint in _checkpoint_list(len(run_log), CHECKPOINT_STEP):
        prefix = run_log[:checkpoint]
        ...
        selected.append(
            EvalTask(
                ...
                selection_label=f"global_best_so_far_t{checkpoint:03d}",
                source_phase=str(best_rec.get("phase", "")),
                global_checkpoint=int(checkpoint),
...
def _select_phase_best_so_far(run_log: List[Dict[str, Any]]) -> List[EvalTask]:
    selected: List[EvalTask] = []

    for phase_name in PHASE_NAMES:
        phase_recs = _phase_records(run_log, phase_name)
        if not phase_recs:
            continue

        for phase_checkpoint in _checkpoint_list(len(phase_recs), CHECKPOINT_STEP):
            prefix = phase_recs[:phase_checkpoint]
            best_rec = min(prefix, key=lambda r: (_train_objective(r), int(r["candidate_id"])))
            best_obj_so_far = min(_train_objective(r) for r in prefix)
            ...
            selected.append(
                EvalTask(
                    selection_family="phase_best_so_far",
                    selection_label=f"{phase_name}_best_so_far_t{global_checkpoint:03d}",
                    source_phase=phase_name,
                    global_checkpoint=int(global_checkpoint),
                    phase_checkpoint=int(phase_checkpoint),
                    source_candidate_id=int(best_rec["candidate_id"]),
                    source_train_objective=float(_train_objective(best_rec)),
                    source_logged_mean_cost=float(best_rec.get("mean_cost", float("nan"))),
                    source_logged_std_cost=float(best_rec.get("std_cost", float("nan"))),
...
def _add_stage_lines() -> None:
    plt.axvline(x=PHASE1_END_TRIAL, linestyle="--", linewidth=1.2)
    plt.axvline(x=PHASE2_END_TRIAL, linestyle="--", linewidth=1.2)
...
def _save_global_family_plot(rows: List[Dict[str, Any]], y_key: str, y_label: str) -> None:
    ...
    plt.ylabel(y_label)
    plt.title(f"Global best-so-far: {y_label}")
...
def _save_phase_family_plot(rows: List[Dict[str, Any]], y_key: str, y_label: str) -> None:
    family_rows = [r for r in rows if r["selection_family"] == "phase_best_so_far"]
    if not family_rows:
        return

    grouped: Dict[str, List[Dict[str, Any]]] = {}
    for row in family_rows:
        grouped.setdefault(str(row["source_phase"]), []).append(row)

    os.makedirs(PLOTS_DIR, exist_ok=True)
    plt.figure(figsize=(10, 5.5))

    for phase_name in PHASE_NAMES:
        phase_rows = grouped.get(phase_name, [])
        phase_rows.sort(key=lambda r: int(r["global_checkpoint"]))
        if not phase_rows:
            continue

        xs = [int(r["global_checkpoint"]) for r in phase_rows]
        ys = [float(r[y_key]) for r in phase_rows]
        plt.plot(xs, ys, marker="o", label=phase_name)

    _add_stage_lines()
    plt.xlabel("Global trial")
    plt.ylabel(y_label)
    plt.title(f"Phase best-so-far: {y_label}")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_DIR, f"phase_best_so_far_{y_key}.png"), dpi=160)
    plt.close()
...
def _save_all_plots(rows: List[Dict[str, Any]]) -> None:
    ...
    for y_key, y_label in metrics:
        ...
        _save_phase_family_plot(rows, y_key, y_label)
...
def main() -> None:
    ...
    phase_best_candidates = _select_phase_best_so_far(run_log)

    all_candidates = global_best_candidates + phase_best_candidates
    all_candidates.sort(
        key=lambda c: (
            int(c.global_checkpoint),
            0 if c.selection_family == "phase_best_so_far" else 1,
            str(c.source_phase),
            int(c.source_candidate_id),
        )
    )
        ...
        [
            {
                ...
                "phase_checkpoint": c.phase_checkpoint,
    ...
    print(f"[INFO] Selected {len(phase_best_candidates)} phase best-so-far candidates.")
    ...
    try:
        ...
        for idx, cand in enumerate(all_candidates, start=1):
            print(
                ...
                f"phase={cand.source_phase} | global_checkpoint={cand.global_checkpoint}"
            )
            ...
            row = {
                ...
                "phase_checkpoint": cand.phase_checkpoint if cand.phase_checkpoint is not None else "",
