...
# ============================================================
STAGE1_GLOBAL_RANDOM_FRACTION = 0.3
...
STAGE3_PLANNER_SIGMA_MAX_FRAC = 0.30
...
ROS_HOME = os.path.expanduser(f"~/.ros_tune_random_{PROFILE_NAME}_{GLOBAL_SEED}")
...
TOP10_PATH = os.path.join(SCRIPT_DIR, f"top10_so_far_random_search_{PROFILE_NAME}.json")
SUMMARY_PATH = os.path.join(SCRIPT_DIR, f"tuning_summary_random_search_{PROFILE_NAME}.json")
RUN_LOG_PATH = os.path.join(SCRIPT_DIR, f"tuning_run_random_search_{PROFILE_NAME}.jsonl")
BEST_JSON_PATH = os.path.join(
    os.path.dirname(CONTROL_PARAM_JSON),
    f"control_param_best_lti_3stage_random_search_{PROFILE_NAME}.json",
)
...
def _weighted_choice_record(rng: random.Random, recs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ranked = sorted(recs, key=lambda r: r["mean_cost"])
...
def build_phase3_startup_joint_anchors(
    ...
    planner_sigmas = {k: PHASE2_SAFE_PLANNER_SIGMA_FRAC_VMAX * (hi - lo) if k == "velocity_planner.v_max"
                      else PHASE2_SAFE_PLANNER_SIGMA_FRAC_MU * (hi - lo)
                      for k, (lo, hi) in PLANNER_BOUNDS.items()}
...
def _register_result(
    ...
    if phase == "phase1_cost_only_random":
        PHASE1_RESULTS.append(rec)
    elif phase == "phase2_planner_only_random":
        PHASE2_RESULTS.append(rec)
    elif phase == "phase3_joint_random":
...
def run_stage1_random_search(
    ...
    for i in range(min(n_trials, len(startup))):
        ...
        print(
            f"[phase1_cost_only_random][startup {i:03d}] "
    ...
    for i in range(len(PHASE1_RESULTS), n_trials):
        ...
        print(
            f"[phase1_cost_only_random][trial   {i:03d}] "
...
def run_stage2_planner_random(
    ...
    for i in range(min(n_trials, len(startup))):
        ...
        print(
            f"[phase2_planner_only_random][startup {i:03d}] "
            f"mean_cost={J:.6g} std={info.get('std_cost', 0.0):.6g} "
            f"crashes={info.get('crashes', 0)}/{info.get('episodes', 0)} "
            f"crit_mult={critical_crash_multiplier:.3f}"
        )
    ...
    for i in range(len(PHASE2_RESULTS), n_trials):
        ...
        print(
            f"[phase2_planner_only_random][trial   {i:03d}] "
            f"kind={proposal_kind:<13} "
            f"mean_cost={J:.6g} std={info.get('std_cost', 0.0):.6g} "
            f"crashes={info.get('crashes', 0)}/{info.get('episodes', 0)} "
            f"crit_mult={critical_crash_multiplier:.3f}"
...
def run_stage3_joint_random(
    ...
    for i in range(min(n_trials, len(startup))):
        ...
        print(
            f"[phase3_joint_random][startup {i:03d}] "
            f"mean_cost={J:.6g} std={info.get('std_cost', 0.0):.6g} "
            f"crashes={info.get('crashes', 0)}/{info.get('episodes', 0)} "
            f"crit_mult={critical_crash_multiplier:.3f}"
        )
    ...
    for i in range(len(PHASE3_RESULTS), n_trials):
        ...
        print(
            f"[phase3_joint_random][trial   {i:03d}] "
            f"kind={proposal_kind:<13} "
            f"mean_cost={J:.6g} std={info.get('std_cost', 0.0):.6g} "
            f"crashes={info.get('crashes', 0)}/{info.get('episodes', 0)} "
            f"crit_mult={critical_crash_multiplier:.3f}"
...
def main() -> None:
    ...
    print("[CFG] Phase2 =", PHASE2_N_TRIALS, "planner-only random trials")
    ...
    print(f"[CFG] STAGE1 global fraction = {STAGE1_GLOBAL_RANDOM_FRACTION}")
    print(f"[CFG] STAGE2 global fraction = {STAGE2_GLOBAL_RANDOM_FRACTION}")
    print(f"[CFG] STAGE3 global fraction = {STAGE3_GLOBAL_RANDOM_FRACTION}")
    ...
    try:
        ...
        print("[PHASE3] startup zawiera center + conservative anchor + planner anchors + cost anchors")
