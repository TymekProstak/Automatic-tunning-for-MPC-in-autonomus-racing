...
ROS_HOME = os.path.expanduser(f"~/.ros_tune_gpbo_{PROFILE_NAME}_{GLOBAL_SEED}")
...
TOP10_PATH = os.path.join(SCRIPT_DIR, f"top10_so_far_gpbo_{PROFILE_NAME}.json")
SUMMARY_PATH = os.path.join(SCRIPT_DIR, f"tuning_summary_gpbo_{PROFILE_NAME}.json")
RUN_LOG_PATH = os.path.join(SCRIPT_DIR, f"tuning_run_gpbo_{PROFILE_NAME}.jsonl")
BEST_JSON_PATH = os.path.join(
    os.path.dirname(CONTROL_PARAM_JSON),
    f"control_param_best_lti_3stage_gpbo_{PROFILE_NAME}.json",
)
...
def build_phase2_startup_planner_only(
    ...
    - koszty są zamrożone
    - exact SAFE_PLANNER
    - local cloud wokół SAFE_PLANNER
    - reszta = wide planner LHS
    """
    rng = random.Random(seed)
    out: List[Dict[str, float]] = []
...
def run_stage1_gpbo(
    ...
    for i in range(min(n_trials, len(startup))):
        ...
        print(
            ...
            f"mean_cost={J:.6g} std={info['std_cost']:.6g} "
...
def main() -> None:
    ...
    print("[CFG] Phase2 =", PHASE2_N_TRIALS, "planner-only GP-BO trials")
    ...
    try:
        ...
        G1 = sorted(PHASE1_RESULTS, key=lambda r: r["mean_cost"])[:max(MIN_G_SIZE, int(math.ceil(GOOD_FRAC * len(PHASE1_RESULTS))))]
        L1 = sorted(PHASE1_RESULTS, key=lambda r: r["mean_cost"])[len(G1):]
        ...
        print("\n================ PHASE 3: JOINT GP-BO ================\n")
        print("[PHASE3] startup = anchors + safe anchor + szeroki planner jitter")
        print("[PHASE3] stage 3 jest najluźniejszy\n")
