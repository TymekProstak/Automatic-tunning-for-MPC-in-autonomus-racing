...
# Config
# ============================================================
WS="${TUNE_WS:-$HOME/Desktop/fs_control_ka_racing/TUNE_BO_TPO}"
PY_SCRIPT_REL="${EVAL_PY_SCRIPT_REL:-src/BO_TPO/eval.py}"

# Dodajemy ścieżkę do logów z tuningu (stąd skrypt bierze kandydatów do testu)
RUN_LOG_JSONL="${EVAL_RUN_LOG_PATH:-$WS/src/BO_TPO/tuning_run_night_4track.jsonl}"

SKIP_BUILD="${SKIP_BUILD:-0}"
ROS_SETUP="${TUNE_ROS_SETUP:-/opt/ros/noetic/setup.bash}"

# Środowisko wirtualne dla bezpieczeństwa zależności
VENV_DIR="${EVAL_VENV_DIR:-$WS/.venv_eval}"
RUN_LOG_DIR="${TEST_RUN_LOG_DIR:-$WS/run_logs}"
mkdir -p "$RUN_LOG_DIR"

STAMP="$(date +%Y%m%d_%H%M%S)"
RUN_LOG="$RUN_LOG_DIR/test_eval_tpe_local_lhs_${STAMP}.log"
...
if [ ! -f "$RUN_LOG_JSONL" ]; then
  ...
  # Informujemy tylko warningiem, w razie gdyby skrypt tworzył/ściągał plik z innego miejsca
fi
...
# To jest kluczowe: skrypt TPE potrzebuje wiedzieć skąd wziąć logi do ewaluacji:
