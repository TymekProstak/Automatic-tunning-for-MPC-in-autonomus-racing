...
if EVAL_PROFILE_NAME == "smoke_3track":
    # --------------------------------------------------------
    # smoke train -> smoke_1track
    # eval on 3 held-out test tracks
    # --------------------------------------------------------
    ...
    DEFAULT_OUTPUT_CANDIDATES_NAME = "tpe_test_candidates_smoke3tracks.json"
    DEFAULT_OUTPUT_CSV_NAME = "tpe_test_results_smoke3tracks.csv"
    DEFAULT_PLOTS_DIR_NAME = "tpe_eval_plots_smoke3tracks"
    ...
    # --------------------------------------------------------
    # night train -> night_4track
    # eval on 7 held-out test tracks
    # --------------------------------------------------------
    ...
    DEFAULT_OUTPUT_CANDIDATES_NAME = "tpe_test_candidates_night7tracks.json"
    DEFAULT_OUTPUT_CSV_NAME = "tpe_test_results_night7tracks.csv"
    DEFAULT_PLOTS_DIR_NAME = "tpe_eval_plots_night7tracks"
...
# ============================================================
# TPE evaluation-side "algorithm thinks"
# ============================================================
...
ROS_MASTER_PORT = int(os.environ.get("EVAL_ROS_MASTER_PORT", "11312"))
START_ROSCORE_IF_NEEDED = True

ROS_HOME = os.path.expanduser(f"~/.ros_eval_tpe_{EVAL_PROFILE_NAME}")
...
def _best_record(records: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    return min(
        records,
        key=lambda r: (_train_objective(r), int(r.get("candidate_id", 10**18))),
    )
...
def _select_algorithm_thinks_schedule(records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for checkpoint_filtered in checkpoints_filtered:
        ...
        if checkpoint_filtered <= STAGE1_END_FILTERED_TRIAL:
            ...
            algo_kind = "phase1_G_center_local_lhs"
            ...
            algo_kind = "phase2_G_center_local_lhs"
            ...
            algo_kind = "phase3_G_center_local_lhs"
        ...
        selected.append(
            {
                ...
                "checkpoint_trial_filtered": int(checkpoint_filtered),
                "checkpoint_trial_global": int(checkpoint_global),
                "source_candidate_id": int(src_rec.get("candidate_id", 0)),
...
def main() -> None:
    ...
    print("[INFO] TPE evaluator reconstructs best_so_far from tuning_run jsonl using objective/robust when available.")
    print("[INFO] algorithm_thinks = stage-aware local-LHS approximation around TPE good set G.")
